...
在长对话/大仓库场景下，`jca` 可能因 **token 触发上下文压缩** 而出现“摘要后再次验证/再次输出”的体验。当前版本在代码层提供 **A+B+C** 组合治理（交付落盘短路、只读验证缓存、只读 `execute_script` 降噪），详细说明见：

- [jca（CodeAgent）防重复执行治理（A+B+C）](../technical/implementation/jca_governance_abc.md)

实践建议：

- 对“已最终交付”的任务，让模型在最终回答中输出结构化 `<JCA_DELIVERY>...</JCA_DELIVERY>`（`code_analysis` 场景已在系统提示词中要求）
- 当你确实需要让代理**重新完整执行**时，在需求里显式包含“重新/重跑/force”等意图，避免被短路机制误判为重复执行
