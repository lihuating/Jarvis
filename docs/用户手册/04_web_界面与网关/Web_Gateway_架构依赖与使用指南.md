# Jarvis Web Gateway 架构、依赖与使用指南

本文档说明 Jarvis Web 体系的整体架构、Server/Client 依赖、启动与使用方式，以及通过 WebSocket 实现远程访问的原理。

---

## 一、整体架构

Jarvis 的 Web 体系是**前后端分离 + 网关桥接**的三层架构，**不是** `jvs` 终端命令自带的网页模式。
...
> **已知限制**：当前版本中 **`jvs` 不支持 `--web-gateway`**，Web 界面创建「通用 Agent」可能失败；创建「代码 Agent」（`jca`）更可靠。
...
**前端连接逻辑**（`src/jarvis/jarvis_service/frontend/src/App.vue`）：
...
适用于多机 Agent 池、异地协作。详见 [分布式网关部署方法](./分布式网关部署方法.md)。
