...
### Q14: 为什么 `jca` 在很长会话里会出现「摘要后再次验证/再次输出」？

**A14:** 这通常是 **上下文压缩** 与 **工具后处理** 叠加后的体验问题：对话过长会触发 token 限制从而生成摘要并清理历史；摘要后模型为稳妥会再次做关键验证（例如 `rg`/读取关键文件）。同时，如果本轮工具调用里包含 `execute_script` 这类“高风险入口”，可能触发更重的后处理链路，从而进一步放大 token 压力。

**建议：**

- 先阅读技术说明：[jca（CodeAgent）防重复执行治理（A+B+C）](../technical/implementation/jca_governance_abc.md)
- 对“已最终交付”的任务，让模型在最终回答中输出 `<JCA_DELIVERY>...</JCA_DELIVERY>`（`code_analysis` 场景已在系统提示词中要求）
- 若你确实需要重新完整执行，在需求里显式写清「重新/重跑/force」等意图
