...
class CodeAgent(Agent):
    ...
    def _build_code_agent_tool_list(
        ...
        base_tools = [
            ...
            "read_code",  # 代码读取工具
            "search_file_content",  # ripgrep 文件内容搜索（Jarvis 独有）
    ...
    def run(
        ...
        try:
            ...
            user_input_for_fp = (
                user_input
                if isinstance(user_input, str)
                else self._convert_to_string(user_input)
            )
...
"""
            ...
            # 结构化交付：解析 <JCA_DELIVERY> 并落盘 marker/delivery
            try:
                if result_str:
                    ...
                    if delivery_obj and bool(delivery_obj.get("delivered")):
                        ...
                        write_marker(
                            ...
                            delivered=True,
                            artifact_type=str(
                                delivery_obj.get("artifact_type") or "unknown"
                            ),
                            artifact_refs=list(delivery_obj.get("artifact_refs") or []),
                            evidence={"head": get_latest_commit_hash()},
                        )
                        write_delivery_artifact(
                            delivery_path,
                            fingerprint=fp,
